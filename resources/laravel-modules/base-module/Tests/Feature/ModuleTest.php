...
test('can see {model-} edit page', function() {
    $this->authenticate();
    ${model_} = {Model}::factory()->create();
    $this->get(route('app.{module}.edit', ${model_}->id))->assertOk();
});

test('can update {model-}', function() {
    $this->authenticate();
    ${model_} = {Model}::factory()->create();
    $this->patch(route('app.{module}.update', ${model_}->id), [
        'name' => 'Joe Smith',
       'email' => 'joe@joe.com'
    ])->assertRedirect(route('app.{module}.index'));
...
test('can delete {model-}', function() {
    $this->authenticate();
    ${model_} = {Model}::factory()->create();
    $this->delete(route('app.{module}.delete', ${model_}->id))->assertRedirect(route('app.{module}.index'));
